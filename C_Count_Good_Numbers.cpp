...
void solve()
{
    ...
    cin >> l >> r;
    ll base = 2 * 3 * 5 * 7;
    ll base_count = get_good(base);
    ll count_r = (r / base) * base_count + get_good(r % base);
    ll count_l = ((l - 1) / base) * base_count + get_good((l-1) % base);
    cout << (r - l + 1) - (count_r - count_l) << endl;
}
