...
void solve() {
    ll n;
    cin >> n;
    vector<vector<ll>> xt(n+1, vector<ll>(2));
    for(int i=1;i<=n;i++) cin >> xt[i][0];
    for(int i=1;i<=n;i++) cin >> xt[i][1];
    vector<ll> xx;
    for(int i=1;i<=n;i++){
        xx.push_back(xt[i][0] - xt[i][1]);
        xx.push_back(xt[i][0] + xt[i][1]);
    }
    ll min = *min_element(xx.begin(), xx.end());
    ll max = *max_element(xx.begin(), xx.end());
